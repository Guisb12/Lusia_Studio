...
### Quiz overflow — REGRAS DE TAMANHO (crítico)
O quiz DEVE caber na zona de conteúdo (480px). Se ultrapassar, o slide está partido. Regras obrigatórias:

**Layout FULL (quiz standalone ou quiz abaixo de contexto):**
- Pergunta: max 2 linhas (font-size 24px). Se mais longa, reformula.
- Opções MC: max 1 linha cada (font-size 20px). Texto curto e direto.
- Max 4 opções MC. Se precisas de mais contexto, usa 2 colunas.
- Gap entre opções: 10px (NÃO 12px) quando há 4 opções.
- Se o slide tem contexto + quiz, o contexto é max 2-3 frases curtas.

**Layout 2 COLUNAS (contexto à esquerda, quiz à direita):**
- Pergunta: font-size 21px, max 2 linhas.
- Opções MC: font-size 18px, max 1 linha cada, gap 8px.
- True/false: font-size 18px nos botões.
- Este é o layout PREFERIDO para quiz — separa contexto do quiz, usa melhor o espaço.

**Regra geral:** Se o quiz tem pergunta longa + 4 opções longas → usa 2 COLUNAS com contexto à esquerda e quiz compacto à direita. NUNCA tentes enfiar tudo em FULL com texto grande — vai overflow.
